...
pairs :: [Pos] -> [Pos] -> [(Pos, Pos)] -> [(Pos, Pos)]
pairs [] _ acc = acc
pairs (x : xs) ps acc =
  let closest :: Pos
      closest = case (drop 1 $ sortOn snd $ map (\y -> (y, distance x y)) $ ps) of
        ((y, _) : _) -> y
        _ -> error "wtf"
   in pairs xs ps ((x, closest) : acc)
...
main = do
  ...
  -- print . length $ ps
  -- print . sortOn negate . map length . solve . pairs' $ ps

  print . product . take 3 . sortBy (flip compare) . map length . solve . pairs' $ ps
