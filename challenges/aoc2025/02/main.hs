...
pairs :: [Int]
pairs = [parseInt (show x ++ show x) | x <- [1 ..]]
...
main = do
  ...
  -- let ranges = S.fromList . concat . parseRanges (contents ++ ",") $ []
  putStr $ show . sum . S.filter (`elem` rangesRaw) $ S.map (\x -> let first = (take ((length $ show x) `div` 2) $ show x) in parseInt (first ++ first)) $ rangesRaw
